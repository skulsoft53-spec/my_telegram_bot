...
# ========== ОТПРАВКА СООБЩЕНИЯ ВО ВСЕ ЧАТЫ ПРИ ЗАПУСКЕ ==========
async def send_to_all_chats(app: Application):
    await asyncio.sleep(5)  # Ждём, пока бот полностью запустится
    async for chat in app.bot.get_updates():
        pass  # Просто для инициализации
    
    # Получаем все чаты, где есть бот
    async for chat in app.bot.get_chat_updates():
        try:
...
def main():
    ...
    # Запускаем отправку сообщений во все чаты
    loop = asyncio.new_event_loop()
    asyncio.set_event_loop(loop)
    loop.run_until_complete(send_to_all_chats(app))
