...
async def start(update: Update, context):
    await update.message.reply_text(
        "👹 **Бот-тролль активен!**\n\n"
        "📌 Команда: `.ку @username` — оскорбляет пользователя (каждое сообщение отдельно)\n"
...
async def cmd_insult(update: Update, context):
    ...
    # Отправляем каждое оскорбление отдельным сообщением
    for insult in insults:
        await update.message.reply_text(f'@{target} {insult}')
        await asyncio.sleep(0.05)  # Очень маленькая задержка, чтобы не ловить флуд-бан
