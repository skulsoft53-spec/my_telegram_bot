...
async def auth_handler(event):
    ...
    if text == '/start':
        if user_client and user_client.is_connected():
            await event.reply("✅ Уже авторизован. Команда: .ку @username")
            return
...
async def command_handler(event):
    ...
    if cmd == 'ку' and len(parts) > 1:
        ...
        try:
            ...
            message_lines = '\n'.join([f'@{target} {insult}' for insult in insults])
            await user_client.send_message(chat_id, message_lines)
...
async def main():
    ...
    print('👉 После авторизации: .ку @username')
