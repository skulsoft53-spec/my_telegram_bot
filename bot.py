...
BOT_TOKEN = '8743970830:AAEA686CW09D4KGCppEOKu16x-CeFECCSg8'
        ...
        if file.startswith('user_') and file.endswith('.session'):
            ...
            try:
                ...
                if await client.is_user_authorized():
                    user_client = client
                    print(f'✅ Загружена сессия для user_id: {user_id}')
                    return True
            except:
                pass
    return False
...
async def handler(event):
    ...
    if chat_id == user_id:
        ...
        # Обработка ввода кода через текстовые сообщения
        if user_id in user_auth_data and user_auth_data[user_id].get('state') != 'password_waiting':
            code = text.strip()
...
async def main():
    ...
    if user_client:
        print('✅ Сессия загружена. Авторизация не требуется.')
    else:
        print('👉 Напиши боту /start, авторизуйся один раз')
