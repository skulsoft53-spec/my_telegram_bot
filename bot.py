        ...
        if file.startswith('user_') and file.endswith('.session'):
            ...
            try:
                ...
                if await client.is_user_authorized():
                    user_client = client
                    print(f'✅ Загружена сохранённая сессия для user_id: {user_id}')
                    return True
                else:
...
async def main():
    ...
    if user_client:
        print('✅ Сохранённая сессия загружена. Авторизация не требуется.')
    else:
        print('👉 Напиши боту /start, авторизуй свой аккаунт (один раз)')
