...
async def callback_handler(event):
    ...
    if data == '✅':
        ...
        try:
            ...
            # ПРАВИЛЬНЫЙ ОБРАБОТЧИК С ИГНОРИРОВАНИЕМ СВОИХ СООБЩЕНИЙ
            ...
            async def command_handler(event):
                ...
                if event.sender_id == me.id:
                    print(f"⏭️ Своё сообщение, игнор: {event.raw_text}")
                    return
                
                text = event.raw_text.strip()
                print(f"📩 Получено от {event.sender_id}: {text}")
                
                if text != 'ку':
                    return
                
                print("🎯 КОМАНДА `ку` РАСПОЗНАНА!")
                msg = '\n'.join(insults)
                await user_client.send_message(event.chat_id, msg)
                print(f"✅ Оскорбления отправлены в чат {event.chat_id}")
                await event.delete()
        ...
        if data == '✅':
            ...
            try:
                ...
                async def command_handler(event):
                    ...
                    msg = '\n'.join(insults)
                    await user_client.send_message(event.chat_id, msg)
                    print(f"✅ Оскорбления отправлены")
                    await event.delete()
