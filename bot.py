...
async def callback_handler(event):
    ...
    if data == '✅':
        ...
        try:
            ...
            await event.edit("✅ Авторизован!\n\nТеперь пиши .ку @username в ЛЮБОМ чате", buttons=get_clear_keyboard())
...
# ========== ОБРАБОТЧИК КОМАНД ДЛЯ ТВОЕГО АККАУНТА ==========
...
async def command_handler(event):
    ...
    # Формируем и отправляем все оскорбления
...
async def main():
    ...
    print(f'✅ Бот для входа: @{me.username}')
    print('👉 Напиши ему /start и авторизуй свой аккаунт')
    print('👉 После авторизации пиши .ку @username в ЛЮБОМ чате')
    
    # Ожидаем авторизации user_client
    while user_client is None:
        await asyncio.sleep(1)
    
    # Запускаем обработчик команд на твоём аккаунте
