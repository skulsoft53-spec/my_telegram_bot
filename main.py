...
LOVE_PHRASES = [
    ...
    "Ты моё вдохновение", "Ты просто прекрасна", "Ты мой свет в любой день", "Ты человек, которого не заменить",
    "Ты моё всё", "Ты дыхание моих чувств", "Ты часть моего мира", "Ты нежность моего сердца",
    "Ты моё утро и мой покой", "Ты чудо, подаренное судьбой", "Ты наполняешь жизнь смыслом",
    "Ты мой покой в шумном мире", "С тобой хочется жить", "Ты делаешь меня счастливым",
    "Ты — моё настоящее", "Ты — лучшее, что со мной случалось", "Ты как солнце после дождя",
    "Ты даришь тепло даже молчанием", "Ты — моя гармония", "Ты — мой дом", "Ты всегда в моих мыслях",
    "Ты — причина моего вдохновения", "Ты приносишь свет туда, где темно", "Ты — мой самый нежный человек",
    "Ты даёшь мне силы", "Ты — мой уют и покой", "С тобой всё имеет смысл", "Ты наполняешь меня радостью",
    "Ты — мой смысл", "Ты — человек, которого хочется беречь", "Ты — счастье, о котором я не просил, но получил",
    "Ты — мой тихий рай", "Ты — мой день и моя ночь", "Ты — нежность, в которой хочется остаться",
    "Ты — самая добрая часть моего сердца", "Ты делаешь жизнь ярче", "Ты — человек, с которым хочется всё",
    "Ты — мой вдохновитель", "Ты — человек, ради которого стоит жить", "Ты — мой внутренний свет",
    "Ты — моё спокойствие в этом мире", "Ты — мечта, ставшая реальностью", "Ты — самое тёплое чувство во мне",
    ...
    "Ты — причина моего счастья", "Ты — человек, с которым время останавливается", "Ты — мой нежный свет",
    "Ты — человек, которого я не хочу терять", "Ты — дыхание моей души", "Ты — человек, который делает мир красивее",
    "Ты — моё вдохновение и покой одновременно", "Ты — нежность, которой не хватает этому миру",
    "Ты — человек, без которого день неполный", "Ты — моя самая добрая мысль"
]
...
# 🔥 Фразы для пылающей любви (90–99%)
HOT_LOVE_PHRASES = [
    "Ваши сердца бьются в одном ритме — и даже звёзды замирают от зависти.",
    "Вы будто созданы друг для друга — между вами искрит сама судьба.",
    "Любовь между вами горит, как вечный огонь, и ничто не сможет её погасить.",
    "Ваши чувства — это не просто пламя, это вулкан, пробуждающий всё вокруг.",
    "Такое совпадение сердец бывает лишь раз в жизни — храните его.",
]

# 💍 Совершенная любовь (100%)
PERFECT_LOVE_PHRASES = [
    "Ваши души нашли друг друга в бесконечности — это совершенная любовь.",
    "Мир может исчезнуть, но эта связь останется вечной.",
    "Когда два сердца бьются так, это уже не любовь — это судьба.",
    "Совершенство — это вы. Ни больше, ни меньше.",
    "Ваша любовь — не просто чувство, а сама суть вселенной.",
]
...
LOVE_LEVELS = [
    ...
    (81, 89, "💘 Это почти любовь — искренняя и сильная."),
    (90, 99, "🔥 Пылающая страсть — ваши чувства за пределами слов!"),
    (100, 100, "💞 Совершенная любовь — вы словно созданы друг для друга."),
]
...
async def start(update: Update, context: ContextTypes.DEFAULT_TYPE):
    await update.message.reply_text(
        ...
        "Команды /on и /off доступны только создателю."
    )
...
async def love_command(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if target.lower() == SIGNATURE_USER.lower():
        ...
        phrase = random.choice(PERFECT_LOVE_PHRASES)
    elif score >= 90:
        phrase = random.choice(HOT_LOVE_PHRASES)
    else:
...
def main():
    ...
    print("💘 LoveBot запущен и готов дарить любовь мгновенно!")
    app.run_polling()
